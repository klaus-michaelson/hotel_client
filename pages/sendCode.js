    ...
    >
        <Space direction='vertical' size="large" style={{width:"100%"}}>
            <Space direction='vertical' style={{width:"100%"}}>
                Code
                <Input placeholder="Code" type="number" value={code} onChange={({target})=>setCode(target.value)}></Input>
                <span style={errorStyle}>{errorCode}</span>
            </Space>
            <Space direction='vertical' style={{width:"100%"}}>
                <Row>
                <Col offset={14} style={{alignContent:'center',justifyContent:'center'}}>
                    <Link href='/changepassword'><Button type="default"
                        onClick={()=>submit()}
                        >submit</Button>
                    </Link>
                </Col>
