    ...
    <div>
      ...
      <div className="mrg-ver-big">
        ...
        <div className="pd row flex-wrap flex-space-ev">
          {Object.entries(room.accessibility).map((item, index) => {
            let value = "";
            if (item[1] === true) {
      ...
      <div className="mrg-ver-big">
        <div className="aln-r">
          ...
          <h2 className="inline">
            ETB <b className="">{room.price}</b>
          </h2>
          <small> / night</small>
        </div>
      </div>
      <div>
        <Button type="primary" onClick={() => changePage("reservation")}>
          Reserve
        </Button>
      </div>
    </div>
...
const RoomDetail = (props) => {
  ...
  room.rooms.map((room) => {
    if (!floors.includes(room.floor)) setFloors(() => [...floors, room.floor]);
  });
