...
import { CalendarOutlined, UsergroupAddOutlined, AimOutlined } from '@ant-design/icons';
...
const ReserveProps = ({ updateViews, data, name }) => {
  ...
  if (name == "hall") {
    comp = (
      <div className="row">
        How Many Peoples? <InputNumber min={10} max={1000} defaultValue={0} bordered={false} />
      </div>
    )
  } else {
    comp = (
...
export class Reservation extends Component {
  ...
  render() {
    console.log(this.props.reserveProps);
    return (
      <div>
        <div className={styles.gBookDropdownBox}>
          <h3 className="mrg-btm-big">Enter Valid Reservation Details</h3>
          <div className='row'>
            <span className='pd'>
              <CalendarOutlined />
            </span>
          ...
          <br />
          <ReserveProps name={this.props.name} data={this.props.data} updateViews={this.updateViews} />
        </div>
